...
public class UICreator : MonoBehaviour
{
        ...
        GameObject parent, UnityAction<GameObject> onClose, CancellationToken token)
    {
        ...
        // 2버튼 팝업 버튼들 생성
        List<(GameObject button, GameObject addImage)> popup2Buttons =
            await CreateButtonsAsync(setting.popup2Buttons, popupBg, token);

        // 각 버튼의 클릭 동작 연결
        foreach (var item in popup2Buttons)
        {
            GameObject btnGo2 = item.button; // addImage는 사용하지 않음
            if (btnGo2 != null && btnGo2.TryGetComponent(out Button btn2))
            {
                // 동일 이름의 ButtonSetting 찾기
                ButtonSetting buttonSetting = setting.popup2Buttons.FirstOrDefault(b => b.name == btnGo2.name);
                if (buttonSetting == null) continue;

                btn2.onClick.AddListener(() =>
                {
                    // targetPopupName이 있으면 해당 서브 팝업을 띄움
                    if (!string.IsNullOrEmpty(buttonSetting.targetPopupName))
                    {
                        IPageWithSubPopups page = parent.GetComponentInParent<IPageWithSubPopups>();
                        PopupSetting[] subPopups = page?.GetSubPopups();

                        if (subPopups != null)
                        {
                            PopupSetting target =
                                subPopups.FirstOrDefault(p => p.name == buttonSetting.targetPopupName);
                            if (target != null)
                            {
                                int idx = Array.IndexOf(subPopups, target);
                                _ = CreatePopupAsync(subPopups, idx, parent, null, token); // 비대기 호출(연쇄 팝업)
                            }
                            else
                            {
                                Debug.LogWarning($"[UICreator] SubPopup not found: {buttonSetting.targetPopupName}");
                            }
                        }
                        else
                        {
                            Debug.LogWarning("[UICreator] No subPopups found on parent page");
                        }
                    }
                });
            }
